...
global {
	...
	int nb_people <- 700;
	int nb_transit_commuters <- 10000;
	...
	float resident_growth_share <- 0.0;
	float commuter_growth_share <- 0.0;
...
float applied_transit_expansion_share <- -1.0;
	...
	init {
		...
		float resident_growth_multiplier <- 1.0 + max([0.0, resident_growth_share]) / 100.0;
		int total_residents <- max([0, int(round(float(nb_people) * resident_growth_multiplier))]);
		float commuter_growth_multiplier <- 1.0 + max([0.0, commuter_growth_share]) / 100.0;
		int total_commuters <- max([0, int(round(float(nb_transit_commuters) * commuter_growth_multiplier))]);
		float requested_share <- min([1.0, max([0.0, transit_expansion_share / 100.0])]);
		int nb_cable_commuters <- int(round(float(total_commuters) * requested_share));
		if empty(cable_commuter_origins) {
			nb_cable_commuters <- 0;
		} else if empty(regular_commuter_origins) {
			nb_cable_commuters <- total_commuters;
		}
		nb_cable_commuters <- min([total_commuters, max([0, nb_cable_commuters])]);
		int nb_regular_commuters <- total_commuters - nb_cable_commuters;
		nb_cable_commuters_actual <- nb_cable_commuters;
		nb_regular_commuters_actual <- nb_regular_commuters;
		actual_transit_expansion_share <- (total_commuters = 0 ? 0.0 : float(nb_cable_commuters) / total_commuters);
		create people number: total_residents {
...
experiment road_traffic type: gui {
	...
	parameter "Resident growth (%)" var: resident_growth_share category: "People" min: 0 max: 50;
	parameter "Commuter growth (%)" var: commuter_growth_share category: "People" min: 0 max: 50;
